...
#                        ax.annotate(s='CAPER\ntrajectory', xy=(x, y), xycoords='data', xytext=(-50, 0), textcoords='offset points', ha='right', path_effects=[pe.withStroke(foreground='w', linewidth=3)], color='r')
                ...
                for timeafterlaunch in range(0, 801, 100):
                    row = rocket_track_2[rocket_track_2[:, 0] == timeafterlaunch, :].flatten()
                    x, y = self.map(row[2], row[1])
                    self.map.plot(x, y, 'b.', ax=ax, markersize=15)
                    ax.annotate(s=str(int(row[0])), xy=(x, y), xycoords='data', xytext=(8, 0), textcoords='offset points', ha='left', va='center', path_effects=[pe.withStroke(foreground='w', linewidth=3)], color='b')
                    if timeafterlaunch == 200:
                        ax.annotate(s='RENU\ntrajectory', xy=(x, y), xycoords='data', xytext=(50, 0), textcoords='offset points', ha='left', path_effects=[pe.withStroke(foreground='w', linewidth=3)], color='b')
